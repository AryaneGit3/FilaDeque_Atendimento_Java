...
public class FormFila extends javax.swing.JFrame {
    ...
    void mostra(){
        listFilaDupla.setText("");
            if(!minhaDeque.isEmpty()) // se minha fila nao estiver vazia 
    ...
    void mostraInvertido(){
        listFilaDupla.setText("");
        Iterator meuIterator = minhaDeque.descendingIterator();
    ...
    private void btnAtenderIniActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAtenderIniActionPerformed
        if(!minhaDeque.isEmpty()){ 
            Pessoa p = minhaDeque.removeFirst(); // remove do inicio 
            txtProx.setText("Em atendimento: "+p.getNome());
            txtProx.setText("Proximo a ser atendido: \n" +p); // printa o prox a ser atendido 
            mostra();            
        }
